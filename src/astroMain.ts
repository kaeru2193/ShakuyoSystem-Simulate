...
export function getPlaEcl(astroDay: number) {
    let data: any[] = []
...
export function getAstroDay(year: number, season: number, point: number, day: number) {
...
export function getEclOrb() {
    ...
    planets.forEach(p => {
        ...
        const orbit: number[][] = new Array(orbRes).fill(0).map((a, idx) => {
...
function calcEquPos(pla: number, days: number) {
    let planetEcl: number[]
    if (pla < 0) {
    ...
    const delta = Math.atan(zEqu / Math.sqrt(xEqu**2 + yEqu**2)) * (180 / Math.PI) //赤緯
    const dist = Math.sqrt(xEqu**2 + yEqu**2 + zEqu**2) //距離
...
function calcEclPos(planet: any, days: number) {
    const [ax, ec, inc, ml, lp, la, per]: number[] = [planet.axs, planet.ecs, planet.incs, planet.mls, planet.lps, planet.las, planet.pers]
